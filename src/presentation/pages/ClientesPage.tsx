  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button leftIcon={<Plus size={16} />}>Novo cliente</Button>}
      />
          ...
          <motion.div key={c.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
            <Card hover className="cursor-pointer p-4" onClick={() => setSelected(c)}>
              ...
              <div className="mt-3 space-y-1 text-xs text-muted-foreground">
                <p className="flex items-center gap-1.5"><MapPin size={12} />{c.district}, {c.city}</p>
      ...
      <ClienteDrawer customer={selected} onClose={() => setSelected(null)} />
    </div>
  );
}

function ClienteDrawer({ customer, onClose }: { customer: Customer | null; onClose: () => void }) {
  ...
  return (
    <Drawer
      ...
      footer={<div className="flex justify-end gap-2"><Button variant="outline">Editar</Button><Button>Novo agendamento</Button></div>}
    >
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-2">
          <ContactRow icon={<Phone size={14} />} value={customer.phone} />
          ...
          <ContactRow icon={<MapPin size={14} />} value={`${customer.street}, ${customer.number} — ${customer.district}, ${customer.city}/${customer.state} · CEP ${customer.cep}`} />
        </div>
